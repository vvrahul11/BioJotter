...
{
  my $openwin_lib = "/usr/openwin/lib";
  require Config;
  $openwin_lib .= "/64" if ($Config::Config{use64bitall} || '') eq 'define';
  Preload(DynaLoader::dl_findfile("-L$openwin_lib",'-lX11'))
    if (NeedPreload() && -d $openwin_lib);
}
